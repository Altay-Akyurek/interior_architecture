...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header
        className={`sticky h-16 top-0 py-0 z-50 w-full bg-primary transition-all ${
          sticky
        ...
        <div className='container mx-auto flex items-center justify-between px-4 py-2'>
          ...
          <ul className='hidden lg:flex grow items-center justify-center gap-3'>
            {headerData.map((item, index) => (
              <HeaderLink key={index} item={item} />
            ))}
          </ul>
          <div className='flex items-center xl:gap-3 lg:gap-2 gap-2'>
            {mounted && (
              <button
                ...
                className='flex h-8 w-8 items-center justify-center text-body-color duration-300 dark:text-white hover:opacity-80 transition-opacity'>
                {theme === 'dark' ? (
                  <svg
                    viewBox='0 0 16 16'
                    className={`h-6 w-6 ${
